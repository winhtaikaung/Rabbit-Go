...
func replaceWithRule(rule string, output string) string {
	...
	for i := range ruleArr {
		// fmt.Printf("From = %v, To = %v", ruleArr[i].From, ruleArr[i].To)
		fromRegex := regexp.MustCompile(string(ruleArr[i].From))

		fmt.Println(fromRegex.ReplaceAllString(output, ruleArr[i].To))
		// fmt.Println(s.Replace(output, string(ruleArr[i].From), string(ruleArr[i].To), -1))
