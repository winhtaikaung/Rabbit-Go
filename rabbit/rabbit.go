...
func replaceWithRule(rule string, output string) string {
	...
	for i := range ruleArr {
		...
		// fmt.Println(i)
		fromRegex := pcre.MustCompile(ruleArr[i].From, 0)
		fmt.Println(fromRegex)
		fmt.Println(string(fromRegex.ReplaceAll([]byte(replaceString), []byte(ruleArr[i].To), 0)))
		// fmt.Println(s.Replace(replaceString, string(ruleArr[i].From), string(ruleArr[i].To), -1))
